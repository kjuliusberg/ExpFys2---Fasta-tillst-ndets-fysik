...
\thispagestyle{empty}
\begin{center}
{\huge\bf Laborationsrapport - Fasta tillståndets fysik:\\}
\vspace{0.2cm}
{\Large\bf 
...
Därefter studerades transmissionsspektrum av kisel. 
...
Laborationen presenterar en metod för att bestämma ett okänt grundämnes kristallstruktur och gitterparamater, med målet att specificera vilket grundämne som studeras.
...
Denna laboration presenterar därmed också ett sätt att mäta transmissionsspektrumet för kisel.

\section{Teori}
% Kristallstrukturer generellt (kanske inledning)

Nedan redovisas relavant teori för laborationen.
...
\begin{equation}
    ...
    \label{Eq: Braggs lag}
\end{equation}
där $d$ är avståndet mellan atomplanen, $\theta$ är den vinkel ingående och utgående strålar bildar med kristallplanen och $\lambda$ är strålningens våglängd. Det är däremot möjligt att reflexer som uppfyller Braggs lag inte ger konstruktiv interferens. Detta beror på ämnets kristallstruktur och dess strukturfaktor, $S_{hkl}$, som definieras enligt
\begin{equation}
    S_{hkl} = \sum_j f_j e^{2\pi i \vec G \cdot \vec r},
\end{equation}
där $\vec G$ är den reciproka gittervektorn, $\vec r$ är ortsvektorn för samtliga atomer i enhetscellen, och $f_j$ är respektive atoms formfaktor. 
Beroende på den kristallstruktur som bestrålas kommer specifika $hkl$ ge en strukturfaktor som är identisk lika med 0, 
vilket korresponderar till en förbjuden reflex och inte ger konstruktiv interferens. 
Olika kristallstrukturer medför olika tillåtna och förbjudna reflexer. 
Exempelvis gäller för koppar, med gitterparameter $a=3,61$ Å, och en FCC-struktur att $h,k,l$ alla måste vara udda eller jämna\cite{Hofmann}.  

En metod för att deducera vilken kristallstruktur ett ämne består av är Debye-Sherrer-metoden, 
som grundar sig i att alla kristallstrukturer har olika tillåtna och förbjudna reflexer. 
Genom att låta ett fint pulveriserat prov bestrålas av röntgenstrålning erhålls med hjälp av en Debye-Sherrer-kamera ett cirkulärt diffraktionsmönster på en film, 
se figur \ref{fig:Debye_Scherrer_kamera}, och $\theta$ kan då beräknas enligt 
 \begin{equation}
     r = 2R\theta,
     \label{Eq:theta från cirkel}
 \end{equation}
där $r$ är radien på en godtycklig cirkel i diffraktionsmönstret och $R$ är kameraradien. 
Genom att studera vilka reflexer som är tillåtna respektive förbjudna kan en kristallstruktur specificeras. 
Genom omformulering av ekvation \ref{Eq: Braggs lag} fås
\begin{equation}
...
Genom att beräkna $\theta$ för alla ringar på kamera-filmen och normalisera dessa erhålls alltså ideelt en heltalsserie, 
som kan jämföras med kända serier för att utesluta vissa kristallstrukturer\cite{X-raylabbPM}.
...
Genom att jämföra varje ring till en uppsättning $hkl$ kan $a$ lösas ut och beräknas från \ref{Eq: Heltalsserie}.
...
En bild över försöksuppställningen visas i figur \ref{fig:prov_i_kamera}, där visas ett kapillärrör i en Debye-Sherrerkamera.
...
För att bestämma transmissionsspektrumet av kisel användes ett färdigt kiselprov, en spektrofotometer av modell Cary 5000 och en spektrofotometer av modell Perkin Elmer 1725. 
...
Dessa behandlades sedan digitalt för större precision, för full beräkning och kod, se appendix \ref{app:kod-debye-sherrer}. 
...
\end{equation}
vilket stämmer överens med de reflexer som förväntas erhållas för en FCC struktur. Gitterparametern beräknades sedan genom att betrakta \ref{Eq: Heltalsserie} som en linjär relation 
...
\end{equation}
vilket stämmer överens med redan känt värde och struktur för koppar.
...
Även medelfelet beräknades genom att ta standardavvikelsen för samtliga spektra och dela med antalet spektra som erhölls. 
Den medelvärderade transmissionsspektrat visas tillsammans med dess medelfel i figur \ref{fig:Transmissionsspektrum}. För full analyskod, se appendix \ref{app:kod-transmission}.
...
\begin{equation}
    4\sin^2(\theta) = \left(\dfrac \lambda {2a}\right)^2 (h^2+k^2+l^2)
\end{equation}
som kan betraktas som en linjär funktion enligt
\begin{equation}
    y = k x + m,
\end{equation}
där $y = 4\sin^2(\theta)$ och $x = (h^2+k^2+l^2)$. För detta användes $\texttt{numpy.polyfit}$ där konstanterna $k = \left(\dfrac \lambda {2a}\right)^2$ och $m$ anpassades 
